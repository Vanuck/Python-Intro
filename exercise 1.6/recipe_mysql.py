...
def create_recipe(conn, cursor):
    ...
    while True:
        try:
            number_of_recipes = int(input("How many recipes would you like to build? "))
    ...
    for i in range(number_of_recipes):
        ...
        ingredients_input = input("  Enter recipe's ingredients, each separated by a comma: ")
...
def update_recipe(conn, cursor):
    ...
    if selected_recipe:
        print(f"Which part of the recipe would you like to update for '{selected_recipe[1]}'?")
    else:
    ...
    while True:
        try:
            ...
            if cursor.fetchone()[0] == 0:
                ...
                confirm = input(f"Are you positive you want to delete '{recipe_name}'? (Yes/No): ").lower()
